			...
			struct stats_record *stats_prev)
{
	...
	/* Assignment#2: Print other XDP actions stats  */
	{
		...
		bytes = (rec->total.rx_bytes) / 1000;
		mbps = (rec->total.rx_bytes - prev->total.rx_packets);


		printf(fmt, action, rec->total.rx_packets, pps, rec->total.rx_bytes, bytes, mbps, period);
	}
}
...
void map_get_value_percpu_array(int fd, __u32 key, struct datarec *value)
{
	...
	if((bpf_map_lookup_elem(fd, &key, values)) != 0) {
		fprintf(stderr, "ERR: failed key:0x%X\n", key);
		return;
	}
