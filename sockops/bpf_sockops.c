 ...
 */
static inline
void extract_key4_from_ops(struct bpf_sock_ops *ops, struct sock_key *key, struct flow_key *flow)
{
    ...
    flow->sip4 = key->sip4;
	flow->dip4 = key->dip4;
	flow->dport = key->dport;
	flow->sport = key->sport;
}
...
void bpf_sock_ops_ipv4(struct bpf_sock_ops *skops)
{
    ...
    extract_key4_from_ops(skops, &key, &flow);
